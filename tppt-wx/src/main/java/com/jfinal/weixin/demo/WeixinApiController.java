...
import com.yqglpt.interceptor.CpLoginInterceptor;
import com.yqglpt.model.User;

import net.sf.json.JSONObject;


//@Before(CpLoginInterceptor.class)
public class WeixinApiController extends ApiController {
	...
	public void deleteMenu() {
		User user=getSessionAttr("honzh_user");
		JSONObject result = new JSONObject();
		if(user!=null&&user.get("user_type").equals(2)){
			ApiResult apiResult = MenuApi.deleteMenu();
			if (apiResult.isSucceed())
				result.put("result", "删除成功");
			else
				result.put("result", "删除失败:"+apiResult.getErrorMsg());
			}
		else{

			result.put("result", "禁止操作！");
		}
	...
	public void createMenu()
	{
		...
		String menuJson = "";
		if (!isParaBlank("menu")) {// 页码
			menuJson = getPara("menu");
		}
//		String str = "{\n" +
//				"    \"button\": [\n" +
//				"        {\n" +
...
//				"    ]\n" +
//				"}";
		ApiResult apiResult = MenuApi.createMenu(menuJson);
		JSONObject result = new JSONObject();
		if (apiResult.isSucceed()){
			...
			result.element("msg",apiResult.getErrorMsg());
		}
	}
