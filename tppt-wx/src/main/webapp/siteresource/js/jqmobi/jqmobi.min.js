    ...
    var jm = (function(O) {
        ...
        p.Event = function(ab, aa) {
            var ac = j.createEvent("Events"),
                Y = true;
            if (aa) {
                for (var Z in aa) { (Z == "bubbles") ? (Y = !!aa[Z]) : (ac[Z] = aa[Z])
                }
            }
        ...
        O.addEventListener("message",
            function(Y) {
                if (Y.source == O && Y.data == "jqm-asap") {
                    Y.stopPropagation();
                    if (W.length > 0) { (W.shift()).apply(F.shift(), e.shift())
                    }
                }
            },
            true);
        ...
        p.isMobile = function(Y) {
            var Z = /^1[3456789]\d{9}$/;
            return Z.test(Y)
        };
        p.isNationMobile = function(Y) {
            var Z = /^\d{7,11}$/;
            return Z.test(Y)
        };
