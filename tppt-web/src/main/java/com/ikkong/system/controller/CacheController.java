...
public class CacheController extends BaseController{
	...
	public void getDeptSelect() {
		...
		List<Map<String, Object>> dept = CacheKit.get(DEPT_CACHE, "dept_all",
				new IDataLoader() {
					public Object load() {
						return Db.init().selectList("select ID,PID,simpleName as TEXT from  TFW_DEPT order by num asc", Record.create(), new AopContext(), Cst.me().getDefaultSelectFactory().deptIntercept());
					}
				});
		StringBuilder sb = new StringBuilder();
		sb.append("<select class=\"form-control\" style=\"margin-left:-3px;cursor:pointer;\" id=\"inputs"
	...
	public void getAreaSelect() {
		final String num = getPara("num");
		List<Map<String, Object>> dept = CacheKit.get(DEPT_CACHE, "area_all",
				new IDataLoader() {
					public Object load() {
						return Db.init().selectList("select ID,PID,simpleName as TEXT from  TFW_DEPT where isarea=2 order by num asc", Record.create(), new AopContext(), Cst.me().getDefaultSelectFactory().deptIntercept());
					}
				});
		StringBuilder sb = new StringBuilder();
		sb.append("<select class=\"form-control\" style=\"margin-left:-3px;cursor:pointer;\" id=\"inputs"
	...
	public void deptTreeList() {
		List<Map<String, Object>> dept = CacheKit.get(DEPT_CACHE, "dept_tree_all_" + ShiroKit.getUser().getId(),
				new IDataLoader() {
					public Object load() {
						return Db.init().selectList("select id \"id\",pId \"pId\",simpleName as \"name\",(case when (pId=0 or pId is null) then 'true' else 'false' end) \"open\" from  TFW_DEPT order by pid,num", Record.create(), new AopContext("ztree"), Cst.me().getDefaultSelectFactory().deptIntercept());
					}
				});
	...
	public void getAreaList() {
		List<Map<String, Object>> dept = CacheKit.get(DEPT_CACHE, "area_all_" + ShiroKit.getUser().getId(),
				new IDataLoader() {
					public Object load() {
						return Db.init().selectList("select * from tfw_dept where isarea=2 order by NUM", Record.create());
					}
				});
		renderJson(json(dept));
	}
	...
	public void safetyLedgerList() {
		List<Map<String, Object>> menu = CacheKit.get(SAFETYLEDGER_CACHE, "safetyLedger_all",
				new IDataLoader() {
					public Object load() {
						return Db.init().selectList("SELECT id, parentid pId, `name`, (select CASE WHEN count(0)>0 then 'true' else 'false' end  from safety_ledger a where a.parentid=safety_ledger.id) `open` from safety_ledger  order by `order`,id");
					}
				});
