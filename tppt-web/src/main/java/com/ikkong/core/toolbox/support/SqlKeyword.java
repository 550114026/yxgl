		...
		try {
			...
			if (StrKit.notBlank(w)) {
				...
				for (String m : mm.keySet()) {
					...
					String filter="";
					if(k.equals(CHECK_NULL)){
						String value=mm.get(m).toString();
						if(value.endsWith("1"))
							filter="("+col+" is  null or "+col+"='')";
						else if(value.endsWith("2"))
							filter=col+" >'' ";
					}else{
						filter=col + getKeyWord(k, m);
					}
					if(filter.length()==0)
						continue;
					if (m.indexOf(OR) == -1) {
