...
public class JqGridFactory extends BaseGridFactory {
	...
	public JqGrid<Map<String, Object>> paginate(Integer page, Integer rows,
			String source, Object paras) {
		if (source.toLowerCase().indexOf("select") == -1) {
			source = Blade.dao().getScript(source).getSql();
		}
		Object listTemp=Db.init().paginate(source, Map.class, paras, page, rows);
		BladePage<Map<String, Object>> list = (BladePage<Map<String, Object>>)listTemp;
		List<Map<String, Object>> _rows = list.getRows();
		long _total = list.getTotal();
		long _page = list.getPage();
		long _records = list.getRecords();
